    ...
    void Update()
    {
        ...
        if (walkInput !=0)
        {
            ...
            if (IsRunning)
            {
                ...
                testNormalizedTime = testTime / runAccelTime;
                testTime -= Time.deltaTime;
                testTime = Mathf.Clamp(testTime, 0, runAccelTime);
            
                testSpeed = Mathf.Lerp(runMaxSpeed, 0, testNormalizedTime);
            }
            // is walking
            else
            {
                testNormalizedTime = testTime / testAccelTime;
                testTime -= Time.deltaTime;
                testTime = Mathf.Clamp(testTime, 0, testAccelTime);
            
                testSpeed = Mathf.Lerp(testMaxSpeed, 0, testNormalizedTime);
            }
        }
        // (walkInput = 0)
        else
        {
            ...
            if (WasRunning)
            {
                testNormalizedTime = testTime / runDeccelTime;
                testSpeed = Mathf.Lerp(0, runMaxSpeed, testNormalizedTime);
            ...
            testTime = Mathf.Clamp(testTime, 0, runDeccelTime);
            testTime -= Time.deltaTime;
        ...
        TeleportChargeTime = cursor.TeleportDistance * TeleportChargeTimeFactor;
    }
...
#region INPUT CALLBACKS
    private void Walk_started(InputAction.CallbackContext context)
    {
        ...
        if (LastPressedWalkTime > 0)
        {
            ...
            testTime = (testSpeed == 0) ? runAccelTime : runAccelTime * (1 - testNormalizedTime);
            //testTime = runAccelTime;
        }
        else
        {
            testTime = (testSpeed == 0) ? testAccelTime : testAccelTime * (1 - testNormalizedTime);
        }
    }
    private void Walk_canceled(InputAction.CallbackContext context)
    {
        ...
        if (!IsRunning)
        {
            LastPressedWalkTime = doubleTapToRunTime;

            testTime = (testSpeed == testMaxSpeed) ? testDeccelTime : testDeccelTime * (1 - testNormalizedTime);
        }
        ...
        if (IsRunning)
        {
            ...
            if (testSpeed == runMaxSpeed)
            {
                testTime = runDeccelTime;
            }

            if (testSpeed < testMaxSpeed)
            {
